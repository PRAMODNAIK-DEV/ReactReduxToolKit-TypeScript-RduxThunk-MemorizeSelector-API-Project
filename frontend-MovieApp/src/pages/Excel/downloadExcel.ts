...
export async function generateExcelFile() {
    ...
    const blankRowIndex = firstTableData.length + 1;
    worksheet.addRow([]);
    ...
    // Calculate widths for the first table
    const firstTableColumns = firstTableData[0].map((_, colIndex) => {
        const maxLength = firstTableData.reduce((max, row) => {
    ...
    // Apply column widths to the first table's columns only
    firstTableColumns.forEach((width, colIndex) => {
        worksheet.getColumn(colIndex + 1).width = width;
    });

    // Calculate widths for the second table
    const secondTableColumns = secondTableHeader.map((_, colIndex) => {
        const allRows = [secondTableHeader, ...secondTableData];
    ...
    // Apply column widths for the second table's columns only
    secondTableColumns.forEach((width, colIndex) => {
        worksheet.getColumn(colIndex + 1).width = Math.max(worksheet.getColumn(colIndex + 1).width || 0, width);
    });
    ...
    const firstTableHeaderRow = worksheet.getRow(firstTableStartRow);
    ...
    const secondTableHeaderRow = worksheet.getRow(secondTableStartRow);
    ...
    if (worksheet.lastRow) { // Ensure lastRow is not undefined
        for (let i = secondTableStartRow + 1; i <= worksheet.lastRow.number; i++) {
